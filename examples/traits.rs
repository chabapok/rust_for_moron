...
Иногда параметризация ведет себя как-то странно. Запись

    fn foo<T: TFoo>(v:T) { ... }

на самом деле означает:

    fn foo<T: TFoo + Sized + 'static>(v: T) { ... }

Поэтому, если мы захотим написать так:

    fn foo<T: TFoo>(v: Box<T>) { ... }

то с удивлением обнаружим, что Box<TFoo> в эту функцию не передать. Нужно писать так:

    fn foo<T: TFoo + ?Sized>(v: Box<T>) { ... }
